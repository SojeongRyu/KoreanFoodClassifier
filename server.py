...
def main():
    ...
    while True:
        ...
        try:
            ...
            # DB에 질의
            idFood = model.predict(Image.open('./img.jpg'))
            result = get_foodInfo(idFood)
            ...
            foodImgFile = open("./output_image/" + str(idFood) + ".jpg", "rb")
            fileSize = os.path.getsize("./output_image/" + str(idFood) + ".jpg")
            foodImg = foodImgFile.read(fileSize)
            foodImgFile.close()
            print(str(fileSize))
            connectionSocket.send((str(fileSize) + "\r\n").encode())
            connectionSocket.sendall(foodImg)
